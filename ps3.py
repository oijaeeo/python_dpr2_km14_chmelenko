...
def get_word_score(word, n):
    ...
    return sum  * (n - len(word)) * max(1, 7 * len(word) - 3)
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while True:
        ...
        try:
            if int(num_of_hands) > 0:
                break
            else:
                print('Please, enter correctly!')
        except:
            print('Please, enter correctly!')
    replay_the_hand_accum = 1
    substitute_letter_accum = 1
    total_score = 0
    for num in range(0, int(num_of_hands)):
        hand = deal_hand(HAND_SIZE)
        print('Current hand:', end=' ')
        display_hand(hand)
        if substitute_letter_accum == 1:
            while True:
                answ = input('Would you like to substitute a letter? (yes or no) ')
        ...
        print('-'*115)
        if replay_the_hand_accum == 1:
            while True:
                answ = input('Would you like to replay the hand? (yes or no): ')
                if answ == 'yes' or answ == 'no':
                    break
                else:
                    print('Enter "yes" or "no"!')
